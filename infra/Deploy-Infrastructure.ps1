...
.EXAMPLE
    ...
    ./infra/Deploy-Infrastructure.ps1 -Mode whatif

.EXAMPLE
    # Local usage - Deploy to a specific subscription
    ./infra/Deploy-Infrastructure.ps1 -Mode deploy -SubscriptionId "your-sub-id"

.EXAMPLE
    # GitHub Actions usage - Deploy with environment variables
    pwsh ./infra/Deploy-Infrastructure.ps1 -Mode deploy -SkipLogin -SkipConfirmation -VerboseOutput

.EXAMPLE
    # Validate with custom parameters file
    ./infra/Deploy-Infrastructure.ps1 -Mode validate -EnvironmentFile "./infra/parameters.test.json"
#>
    ...
    try {
        ...
        if ($VerboseOutput) {
            ...
            $parameters.GetEnumerator() | ForEach-Object {
                if ($_.Key -like "*password*" -or $_.Key -like "*secret*" -or $_.Key -like "*token*") {
...
function Build-DeploymentParameters {
    ...
    # Create a temporary parameters file for deployment
    $tempParameters = @{
    ...
    if (-not $tempParameters.parameters.ContainsKey('repositoryUrl')) {
        $tempParameters.parameters['repositoryUrl'] = @{ 'value' = $RepoUrl }
    }
    if (-not $tempParameters.parameters.ContainsKey('repositoryBranch')) {
...
function Test-BicepTemplate {
    ...
    # Note: No cleanup here - let the calling function handle cleanup
}
...
try {
    ...
    try {
        ...
        if ($parameterFile -and (Test-Path $parameterFile) -and $parameterFile.Contains("temp-deployment-params")) {
            ...
            Write-Verbose "Cleaned up temporary parameters file: $parameterFile"
        }
    }
