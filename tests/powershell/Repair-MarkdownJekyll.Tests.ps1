...
Describe "Repair-MarkdownJekyll" {
    ...
    BeforeAll {
        ...
        New-Item -ItemType Directory -Path (Join-Path $script:TempPath "collections/_blogs") -Force | Out-Null
        New-Item -ItemType Directory -Path (Join-Path $script:TempPath "collections/_news") -Force | Out-Null
    ...
    Context "Category Validation" {
        It "Should require at least one category" {
            $testFile = Join-Path $script:TempPath "collections/_blogs/no-categories.md"
            $testContent = @"
---
layout: post
...
"@
        ...
        It "Should handle multiple categories properly" {
            $testFile = Join-Path $script:TempPath "collections/_blogs/multi-categories.md"
            $testContent = @"
---
layout: post
...
"@
        ...
        It "Should handle single category in array format" {
            $testFile = Join-Path $script:TempPath "collections/_blogs/single-category.md"
            $testContent = @"
---
layout: post
...
"@
        ...
        It "Should not quote frontmatter skip keys (excerpt_separator, tags, categories, date)" {
            $testFile = Join-Path $script:TempPath "collections/_blogs/2025-01-01-test.md"
            $testContent = @"
---
layout: post
title: Test Post
...
"@
        ...
        It "Should add viewing_mode when missing" {
            $testFile = Join-Path $script:TempPath "collections/_blogs/test-no-layout.md"
            $testContent = @"
---
layout: post
title: Test Post
...
"@
        ...
        It "Should update viewing_mode when different" {
            $testFile = Join-Path $script:TempPath "collections/_blogs/test-viewing-mode.md"
            $testContent = @"
---
layout: post
title: Test Post
...
"@
    ...
    Context "Date Format Repair" {
        It "Should convert YYYY-MM-DD to full datetime format" {
            $testFile = Join-Path $script:TempPath "collections/_blogs/2025-01-01-date-test.md"
            $testContent = @"
---
layout: post
title: Test Post
...
"@
        ...
        It "Should fix timezone format from +0000 to +00:00" {
            $testFile = Join-Path $script:TempPath "collections/_blogs/2025-01-01-timezone-test.md"
            $testContent = @"
---
layout: post
title: Test Post
...
"@
        ...
        It "Should leave correct date format unchanged" {
            $testFile = Join-Path $script:TempPath "collections/_blogs/2025-01-01-correct-date.md"
            $testContent = @"
---
layout: post
title: Test Post
...
"@
        ...
        It "Should rename file to match date in frontmatter" {
            $testFile = Join-Path $script:TempPath "collections/_blogs/wrong-date-title.md"
            $testContent = @"
---
layout: post
title: Test Post
...
"@
            ...
            $expectedFile = Join-Path $script:TempPath "collections/_blogs/2025-01-01-wrong-date-title.md"
...
"@
    ...
    Context "Edge Cases and Error Handling" {
        It "Should handle empty files gracefully" {
            $testFile = Join-Path $script:TempPath "collections/_blogs/empty.md"
        ...
        It "Should handle files without frontmatter" {
            $testFile = Join-Path $script:TempPath "collections/_blogs/no-fm.md"
            $testContent = @"
# Just a heading
...
"@
        ...
        It "Should handle malformed frontmatter gracefully" {
            $testFile = Join-Path $script:TempPath "collections/_blogs/malformed.md"
            $testContent = @"
---
layout: post
...
"@
    ...
    Context "File Processing Results" {
        It "Should report fixed files count correctly" {
            ...
            $testFile1 = Join-Path $script:TempPath "collections/_blogs/test1.md"
            $testFile2 = Join-Path $script:TempPath "collections/_blogs/test2.md"
