...
public class MobileNavigationTests : PlaywrightTestBase
{
    ...
    public async Task MobileMenu_OverlayClick_ClosesMenu()
    {
        ...
        {
            await overlay.ClickAsync(new() { Position = new() { X = 10, Y = 200 }, Timeout = BlazorHelpers.E2ERetryWindowMs });
            await Assertions.Expect(mobileMenuOpen).ToHaveCountAsync(0, new() { Timeout = BlazorHelpers.E2ERetryWindowMs });
        });
    }
    ...
    public async Task MobileMenu_HamburgerToggle_ClosesOpenMenu()
    {
        ...
        var mobileMenuOpen = Page.Locator(".mobile-menu.open");
        await hamburger.ClickAndExpectAsync(async () =>
            await Assertions.Expect(mobileMenuOpen).ToBeVisibleAsync(new() { Timeout = BlazorHelpers.E2ERetryWindowMs }));
