...
CSS files are defined in `TechHub.Web.Configuration.CssFiles.All` and referenced in App.razor via `@Assets["path"]` for fingerprinted URLs.

- **All environments**: Each CSS file gets a fingerprinted URL with a content hash in the filename (e.g., `base.e3opgp91n1.css`)
- **Cache busting**: When file content changes, the URL changes automatically — browsers fetch the new version without needing a hard refresh
- **No bundling**: HTTP/2 multiplexing makes individual files as fast as a bundle, while fingerprinting provides reliable cache invalidation
