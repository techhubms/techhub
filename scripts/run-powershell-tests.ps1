...
# PowerShell Test Runner Script for Tech Hub Content Processing Scripts
...
function Get-ProjectRoot {
    ...
    $sourceRootPath = Join-Path $PSScriptRoot "content-processing/functions/Get-SourceRoot.ps1"
    ...
    while ($currentPath -and $currentPath -ne [System.IO.Path]::GetPathRoot($currentPath)) {
        ...
        if (Test-Path (Join-Path $currentPath "TechHub.slnx")) {
            return $currentPath
        }
...
function Invoke-PowerShellTests {
    ...
    Write-ColoredOutput "Running PowerShell tests for content processing scripts..." $Blue
    
    # Determine test directory path - now in /tests/powershell
    $testDirectory = Join-Path $projectRoot "tests/powershell"
    ...
    if (-not (Test-Path $testDirectory)) {
        ...
        Write-ColoredOutput "💡 Expected directory structure: tests/powershell/*.Tests.ps1" $Yellow
        return 1
    }
...
function Invoke-PowerShellTestsRunner() {
    ...
    try {
        ...
        Write-ColoredOutput "Tech Hub PowerShell Test Runner (Content Processing Scripts)" $Blue
