...
if (-not $SkipRestore) {
    ...
    elseif ($Target -eq 'staging') {
        ...
        foreach ($app in $stagingApps) {
            ...
            az containerapp stop --name $app --resource-group $stagingRg --output none 2>&1 | Out-Null
            if ($LASTEXITCODE -ne 0) {
                Write-Host "   [WARN] Could not stop $($app) (app may not exist yet)" -ForegroundColor Yellow
            }
        }
    ...
    if ($Target -eq 'staging') {
        ...
        foreach ($app in $stagingApps) {
            ...
            az containerapp start --name $app --resource-group $stagingRg --output none 2>&1 | Out-Null
            if ($LASTEXITCODE -ne 0) {
                Write-Host "   [WARN] Could not start $($app) (app may not exist yet)" -ForegroundColor Yellow
            }
        }
