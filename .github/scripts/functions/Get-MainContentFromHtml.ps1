function Get-MainContentFromHtml {
    ...
    try {
        ...
        switch -Regex ($hostName) {
            ...
            "^www\.reddit\.com$" {
                # Reddit processing
                Write-Verbose "Processing Reddit HTML from: $SourceUrl"
                
                # Reddit uses divs with IDs matching pattern .*post.*-content for main content
                # Use balanced extraction for each matching div
                $redditContentPattern = '<div[^>]*\s+id="[^"]*post[^"]*-content"[^>]*>'
                $contentMatches = [regex]::Matches($InputHtml, $redditContentPattern, [System.Text.RegularExpressions.RegexOptions]::Singleline)
                
                if ($contentMatches.Count -gt 0) {
                    $extractedContent = ""
                    foreach ($match in $contentMatches) {
                        # Create a regex pattern that matches this specific opening tag
                        $specificId = ($match.Value -replace '.*id="([^"]*)".*', '$1')
                        $specificPattern = '<div[^>]*\s+id="' + [regex]::Escape($specificId) + '"[^>]*>'
                        $balancedContent = Get-BalancedHtmlContent -InputHtml $InputHtml -TagName "div" -TagPattern $specificPattern
                        
                        if (-not [string]::IsNullOrWhiteSpace($balancedContent)) {
                            $extractedContent += $balancedContent.Trim() + "`n`n"
                        }
                    }
                    
                    $extractedContent = $extractedContent.Trim()
                    if (-not [string]::IsNullOrWhiteSpace($extractedContent)) {
                        Write-Verbose "Successfully extracted Reddit content from $($contentMatches.Count) post content divs using balanced extraction ($(($extractedContent.Length)) characters)"
                        return $extractedContent
                    }
                }
                
                #Only in case of Reddit, do NOT throw, we'll just skip the post later
                Write-Warning "⚠️ Reddit main content extraction from HTML did not result in any content"
                return ""
            }
            "^www\.youtube\.com$" {
