...
test.describe('UI/UX Filter Components', () => {
  ...
  // Test 9: Button Height and Spacing Consistency
  test('should have consistent height and spacing between all filter buttons', async ({ page }) => {
    const testPages = [
      { url: '/ai/news.html', name: 'AI News Collection' },
      { url: '/ai/posts.html', name: 'AI Posts Collection' },
      { url: '/github-copilot/community.html', name: 'GitHub Copilot Community' }
    ...
    for (const testPage of testPages) {
      console.log(`\n🧪 Testing button consistency on: ${testPage.name}`);

      await navigateAndVerify(page, testPage.url);

      // Get all filter buttons (both date and tag filters)
      const allFilterButtons = page.locator('.tag-filter-btn');
      const buttonCount = await allFilterButtons.count();

      if (buttonCount > 1) {
        console.log(`📊 Analyzing ${buttonCount} filter buttons for consistency`);

        // Collect button measurements
        ...
        // Analyze height consistency - ALL buttons must be identical height
        const heights = buttonMetrics.map(m => m.height);
        const uniqueHeights = [...new Set(heights)];

        console.log(`📏 Button heights: ${heights.slice(0, 10).map(h => h.toFixed(1)).join(', ')}${heights.length > 10 ? '...' : ''}`);
        console.log(`📊 Found ${uniqueHeights.length} unique height(s): ${uniqueHeights.map(h => h.toFixed(1)).join(', ')}px`);

        // STRICT requirement: All buttons must have identical height
        expect(uniqueHeights.length).toBe(1);
        console.log(`✅ All ${buttonCount} buttons have identical height: ${uniqueHeights[0].toFixed(1)}px`);

        // Analyze horizontal spacing between adjacent buttons on same line
        const sameLineSpacings = [];

        for (let i = 1; i < buttonMetrics.length; i++) {
          const prev = buttonMetrics[i - 1];
          const current = buttonMetrics[i];

          // Check if buttons are on the same horizontal line (within 2px vertical tolerance)
          const sameLine = Math.abs(current.top - prev.top) <= 2;

          if (sameLine) {
            // Calculate gap: distance between right edge of previous button and left edge of current button
            const gap = current.left - prev.right;

            sameLineSpacings.push({
        ...
        if (sameLineSpacings.length > 0) {
          console.log(`📏 Horizontal spacing analysis between ${sameLineSpacings.length} adjacent same-line button pairs:`);

          // Log individual gaps for debugging
          sameLineSpacings.forEach((spacing, idx) => {
            if (idx < 10) { // Show first 10 for debugging
              console.log(`  Button ${spacing.fromIndex} → Button ${spacing.toIndex}: ${spacing.gap.toFixed(1)}px`);
            }
          });

          const gaps = sameLineSpacings.map(s => s.gap);
          const uniqueGaps = [...new Set(gaps.map(g => Math.round(g * 10) / 10))]; // Round to 0.1px
          const minGap = Math.min(...gaps);
          const maxGap = Math.max(...gaps);

          console.log(`↔️ Horizontal spacing: min=${minGap.toFixed(1)}px, max=${maxGap.toFixed(1)}px`);
          console.log(`📊 Found ${uniqueGaps.length} unique spacing(s): ${uniqueGaps.map(g => g.toFixed(1)).join(', ')}px`);

          // STRICT requirement: All gaps between same-line buttons must be identical
          expect(uniqueGaps.length).toBe(1);
          console.log(`✅ All same-line button gaps are identical: ${uniqueGaps[0].toFixed(1)}px`);
        } else {
          console.log('ℹ️ No same-line button pairs found - all buttons are on separate lines');
        }

        console.log('✅ Button height and spacing consistency verified');
      } else {
        console.log(`ℹ️ Only ${buttonCount} button(s) found - insufficient for consistency analysis`);
      }
    }
  });
