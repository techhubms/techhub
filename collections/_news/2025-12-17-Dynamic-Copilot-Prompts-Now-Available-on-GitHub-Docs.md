---
layout: "post"
title: "Dynamic Copilot Prompts Now Available on GitHub Docs"
description: "This announcement describes the introduction of dynamic GitHub Copilot prompts on select GitHub Docs pages. The feature streamlines developer workflows by allowing direct access to prefilled Copilot prompts from documentation. Three types of prompts are supported: inline prompts, prompt blocks, and prompt blocks with context. The update improves efficiency by removing the need to manually copy/paste, and provides integration between developer documentation and Copilot's coding assistance."
author: "Allison"
excerpt_separator: <!--excerpt_end-->
canonical_url: "https://github.blog/changelog/2025-12-17-dynamic-copilot-prompts-on-github-docs"
viewing_mode: "external"
feed_name: "The GitHub Blog"
feed_url: "https://github.blog/changelog/feed/"
date: 2025-12-17 15:19:18 +00:00
permalink: "/news/2025-12-17-Dynamic-Copilot-Prompts-Now-Available-on-GitHub-Docs.html"
categories: ["AI", "GitHub Copilot"]
tags: ["AI", "AI Integration", "Code Readability", "Coding Assistant", "Copilot", "Copilot Chat", "Copilot Prompts", "Developer Productivity", "Documentation Tools", "GitHub Copilot", "GitHub Docs", "Improvement", "Inline Prompts", "News", "Prompt Blocks"]
tags_normalized: ["ai", "ai integration", "code readability", "coding assistant", "copilot", "copilot chat", "copilot prompts", "developer productivity", "documentation tools", "github copilot", "github docs", "improvement", "inline prompts", "news", "prompt blocks"]
---

Allison shares news about dynamic GitHub Copilot prompts on GitHub Docs, highlighting new workflow integrations that let developers launch prefilled Copilot prompts directly from documentation pages.<!--excerpt_end-->

# Dynamic Copilot Prompts Now Available on GitHub Docs

GitHub has rolled out a new feature that lets you launch GitHub Copilot prompts directly from select pages on [GitHub Docs](https://docs.github.com).

## Key Features

- **Dynamic Copilot Prompts:** Moving seamlessly from documentation to [Copilot](https://github.com/copilot) is now possible, eliminating the manual step of copying and pasting prompts.
- **Prompt Types:**
  - **Inline Prompts:** Send general questions to GitHub Copilot directly from the docs.
  - **Prompt Blocks:** Open specific GitHub-related prompts in Copilot.
  - **Prompt Blocks with Context:** Send a prompt alongside a relevant code block to Copilot for more contextual assistance.
- **Improved Workflow:** This integration allows developers to receive coding assistance or suggestions straight from the documentation, streamlining the process of getting help and staying productive.

## Try It Out

To experience dynamic Copilot prompts, check out the [tutorial on improving code readability](https://docs.github.com/copilot/tutorials/copilot-chat-cookbook/refactor-code/improve-code-readability#example-prompt).

By embedding Copilot integration directly into documentation, GitHub continues to focus on developer efficiency and seamless coding experiences.

---

*Posted by Allison*

This post appeared first on "The GitHub Blog". [Read the entire article here](https://github.blog/changelog/2025-12-17-dynamic-copilot-prompts-on-github-docs)
