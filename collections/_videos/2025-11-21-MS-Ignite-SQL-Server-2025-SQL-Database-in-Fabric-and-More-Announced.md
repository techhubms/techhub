---
external_url: https://www.youtube.com/watch?v=-6RoQm6Wwyo
title: 'MS Ignite: SQL Server 2025, SQL Database in Fabric, and More Announced'
author: Microsoft Developer
feed_name: Microsoft Developer YouTube
date: 2025-11-21 20:06:07 +00:00
tags:
- Azure SQL
- Azure SQL DB
- Cloud Computing
- Cloud Data Platform
- Data Analytics
- Data Engineering
- Data Exposed
- Database Development
- Dev
- Development
- GA Announcements
- Microsoft
- Microsoft Developer
- Microsoft Fabric
- Microsoft Ignite
- SQL
- SQL Server
- Tech
- Technology
section_names:
- azure
- ml
---
Presented by Microsoft Developer, this video covers all key SQL-related product announcements from Microsoft Ignite 2025, including SQL Server 2025 GA and SQL DB's integration with Microsoft Fabric.<!--excerpt_end-->

{% youtube -6RoQm6Wwyo %}

# MS Ignite: SQL Server 2025 GA, SQL DB in Fabric GA & More

Discover everything new in the Microsoft SQL technology space as announced at Microsoft Ignite 2025.

## Key Announcements

- **SQL Server 2025 General Availability (GA)**: The latest version, focusing on improved scalability, cloud-native integrations, and new features for enterprise data management.
- **SQL Database in Microsoft Fabric GA**: SQL Database now integrates with Microsoft Fabric, opening up analytics and data engineering capabilities in Fabric's unified platform.
- **Azure SQL Updates**: Announcements include service enhancements and developer resources for Azure SQL.

## Resources

- [One SQL Resource Hub](https://aka.ms/onesql)
- [Data Exposed Videos](https://aka.ms/dataexposedyt)

## Connect and Learn More

- Follow Anna Hoffman on [Twitter](https://twitter.com/AnalyticAnna)
- [Azure SQL Twitter](https://aka.ms/azuresqltw)
- [Microsoft Azure SQL YouTube](https://aka.ms/msazuresqlyt)
- [Microsoft SQL Server YouTube](https://aka.ms/mssqlserveryt)
- [Microsoft Developer YouTube](https://aka.ms/microsoftdeveloperyt)

## Hashtags

- #AzureSQL #SQL #LearnSQL

---
This episode offers a concise rundown of critical SQL innovations for cloud professionals, developers, and data engineers. Watch more Data Exposed episodes for deeper dives into technical features and tutorials.
