-- Migration 042: Backfill roundup created_at from slug date
-- Date: 2026-05-24
...
SET    created_at = (to_date((regexp_match(slug, '-(\d{4}-\d{2}-\d{2})$'))[1], 'YYYY-MM-DD')
                       + TIME '08:00:00') AT TIME ZONE 'UTC',
       updated_at = NOW()
